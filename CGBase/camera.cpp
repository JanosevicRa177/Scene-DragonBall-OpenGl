...
Camera::Camera() {
    mPosition = glm::vec3(0.0f, 1.2f, -3.0f);
...
void
Camera::SetYaw(float horizontalAngle) {
    mYaw += horizontalAngle;
}
...
void
Camera::SetPitch(float verticalAngle) {
    mPitch += verticalAngle;
}

void 
Camera::Rotate(float dx, float dy, float dt) {
    float RotateVelocity = mLookSpeed* 1.2 * dt;
